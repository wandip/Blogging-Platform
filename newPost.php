...
<br>
<i class="material-icons">image</i> Cover-Image(#) : <input type=file id="myFile" accept="image/*" data-type='image' onchange="checkIfImage()" name="photo" required >

<i class="material-icons">insert_link</i> Hyperlink : <input class="hide" type=checkbox onchange="addattr()">
  
<div class="ex" style="display: none">

	<br>
	<label for="hlink">URL :</label>
	<input type = text id="hlink" name="hlink" placeholder="Enter URL here">
...
<p class="formfield">
<label style=" vertical-align: top;" for="postt"> Post(*) :</label>

<textarea placeholder="Write Post here..." id="postt" name="content" required></textarea>
</p>
...
<?php
  if (isset($_POST['postbutton'])) {
    $query = "insert into Posts(user_id,title,sub_title,content,Date) values (?,?,?,?,?)";
    echo $_POST['title'];
    echo $_POST['subtitle'];
$result = $db->updateSql($query, array($user_id, $_POST['title'],$_POST['subtitle'], $_POST['content'],date("Y/m/d")), $response);
if($result){
